...
public class Manager {
    ...
    public static Persona addPersona(String dni, String nom, String telf) {
        Session session = factory.openSession();
        Transaction tx = null;
        Persona result = null;
        try {
            tx = session.beginTransaction();
            result = new Persona(dni, nom, telf);
            session.save(result); 
            tx.commit();
        } catch (HibernateException e) {
            if (tx!=null) tx.rollback();
    ...
    public static void updateBiblioteca(long bibliotecaId, String nom, String ciutat, Set<Llibre> llibres) {
        ...
        try {
            ...
            obj.setBibliotecaId(bibliotecaId);
            obj.setNom(nom);
